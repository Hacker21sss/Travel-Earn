...
module.exports.respondToConsignmentRequest = async (req, res) => {
  try {
    ...
    if (!travelId || !consignmentId || !response) {
      return res
        .status(400)
        .json({ message: "travelId, consignmentId, and response are required" });
    }
    ...
    if (travelHistory) {
      return res
        .status(400)
        .json(["already accepted"] );
    }
    if (!travelHistoryBasic) {
      return res
        .status(404)
        .json({ message: "Travel history not found for the given travelId" });
    }
    ...
    if (
      ...
      return res.status(400).json({
        message: `This consignment has already been ${
          book.status || consignment.status
        }.`,
      });
    }
    ...
    if (response.toLowerCase() === "accept") {
      ...
      try {
        const [smsResponse, smsResponse1] = await Promise.all([
          axios.get("https://sms6.rmlconnect.net:8443/bulksms/bulksms", {
            params: {
              username: "timestring",
              password: encodeURIComponent("c7*HKy)6"),
              type: 0,
              dlr: 1,
              destination: consignment.phoneNumber,
              source: "TMSSYS",
              message,
              entityid: "1701173330327453584",
              tempid: "1707173408029753777",
            },
          }),
          axios.get("https://sms6.rmlconnect.net:8443/bulksms/bulksms", {
            params: {
              ...
              type: 0,
              dlr: 1,
              destination: consignment.recieverphone,
              source: "TMSSYS",
              message: message1,
              entityid: "1701173330327453584",
              tempid: "1707173408034076405",
            },
          }),
        ]);
      ...
      //         status: "approved",
      //         travelId: book.travelId,
      //         consignmentId:book.consignmentId,
      //         earning:book.earning
      ...
      // ),
        ...
        Consignment.updateOne(
          ...
          console.log(
            `Consignment OTPs updated for ${consignmentId}: Sender OTP - ${senderOtp}, Receiver OTP - ${receiverOtp}`
          );
          return result;
        }),
        ...
        ConsignmentHistory.updateOne(
          { consignmentId },
          {
            $set: {
              ...
              sotp: senderOtp,
              rotp: receiverOtp,
              consignmentpickuptime: travelHistoryBasic.expectedStartTime, // Use travelHistoryBasic
              consignmentdelivertime: travelHistoryBasic.expectedendtime, // Use travelHistoryBasic
            },
            $push: {
              traveldetails: {
                travelId: book.travelId,
      ...
      const [
        savedConsignment,
        updateConsignmentResult,
        updateRequestResult,
        updateHistoryResult,
        updateTravelResult,
      ] = results;

      console.log("✅ Consignment saved:", savedConsignment);
      console.log("📝 Consignment updated:", updateConsignmentResult);
      console.log("🔄 RequestForCarry updated:", updateRequestResult);
      console.log("📜 ConsignmentHistory updated:", updateHistoryResult);
      console.log("🚕 TravelHistory updated:", updateTravelResult);
      ...
      const notification = new Notification({
        ...
        pickuptime: travelHistoryBasic.expectedStartTime, // Use travelHistoryBasic
        dropofftime: travelHistoryBasic.expectedendtime // Use travelHistoryBasic
      });

      await notification.save();
      let io = getIO();
      io.emit("bookingAccepted", { travelId, consignmentId });
      console.log("bookingAccepted event emitted");

      return res
        .status(200)
        .json({ message: "Consignment accepted", senderOtp, receiverOtp });
    } else if (response.toLowerCase() === "reject") {
      await Promise.all([
        Consignment.updateOne(
          { consignmentId },
      ...
      let io = getIO();
      io.emit("bookingrejected", { travelId, consignmentId });
      console.log("bookingrejected event emitted");
